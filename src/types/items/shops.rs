...
{
    fn borrow_inventory(&self) -> &Inventory { &self.inventory }

    fn get_ptr(&self) -> *const Shop { self as *const PersistentShop }

    fn sell_to_rate(&self) -> f32 { 0.0 }

    fn buy_from_rate(&self) -> f32 { 1.0 }
...
impl Shop for BlacksmithShop
{
    ...
    fn get_ptr(&self) -> *const Shop { self as *const BlacksmithShop }

    fn sell_to_rate(&self) -> f32 { 0.6 }
    ...
    fn restock(&self)
    {
        ...
        {
            self.inventory.add_item(item_settings::rand_weapon(None, self.town_num), None);
        }
    }
}