use crate::types::{
    effects::Effect,
    entities::{mobs::Mob, npcs::NPC, players::Player},
    items::{
        self, bows::Bow, display_info::ItemDisplayInfo, inventories::Inventory, swords::Sword,
    },
};
...
pub trait Area: EntityHolder + AreaTools {
    ...
    fn get_movements(&self, _player: &PlayerMeta, responses: &mut Vec<Response>) {
        ...
        for coordinates in connections {
            ...
            let text = if num_connections == 1 {
                ...
                format!("Go {}: {}", direction, get_new_area_title(coordinates))
            };
    ...
    fn get_entity_interactions(&self, player: &PlayerMeta, responses: &mut Vec<Response>) {
        ...
        for entity in entities.iter() {
            if entity.get_id() == player.get_player_id() {
                continue;
            }

            if let Some(text) = entity.get_response_text(player) {
                let accessor = entity.get_accessor();
                responses.push(Response::_get_entity_dialogue(text, accessor));
            }

            if entity.get_type() == "player" {
                let sender = player.get_name();
                let receiver = entity.get_id();
                let wave = Response::_action_only(
                    format!("Wave to {}.", entity.get_name()),
                    move |p| {
                        let msg = *choose(&[
                        "<name> says hello!",
                        "<name> says hi!",
                        ...
                        let formatted =
                            text::apply_replacements(msg, &[("<name>", sender.clone())]);

                        temp_add_short_message(receiver, &formatted);
    ...
    fn get_commands(&self, player: &PlayerMeta, commands: &mut Vec<Command>) {
            ...
            .expect("Player  no longer exists.")
        {
            commands.push(Command::simple("s", "Use your secondary item.", |_, p| {
                p.entity(|e| e.use_secondary()).expect("Player data no longer exists.");
            }));
        }
    }
    ...
    fn _get_dialogue(&self, player: &PlayerMeta) -> Dialogue {
...
fn get_new_map(town_num: usize, player: &PlayerMeta) -> String {
...
pub trait EntityHolder {
    ...
    // These should look nicer in-use.
...
pub trait Entity {
    ...
    fn _get_dialogue(&self, _player: &PlayerMeta) -> Option<Dialogue> {
        None
    }

    fn goto_dialogue(&self, marker: u8, player_id: usize) -> Option<Dialogue> {
        self._goto_dialogue(marker, &*access::player_meta(player_id))
    }

    fn _goto_dialogue(&self, _marker: u8, _player: &PlayerMeta) -> Option<Dialogue> {
        None
    }
